						...
						<ul>
							...
							<li><font color="#000" style="line-height:30px;"
								class="font_input"> &nbsp;&nbsp;您好，我们很高兴的通知您,<input
									...
									class="showJob">(职位),联系方式<input type="text" id="contactPhone" style="width:115px;">。 </span></font>
							</li>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<c:forEach items="${divPage.dataMap['jobApplications']}" var="jobApplication"
										...
										<tr align="center" id="${status.count}" height="29"
											...
											<td align="center" class="r8""><a
												...
												'${divPage.dataMap['resumes'][jobApplication.id].locationPlace}',
													'${divPage.dataMap['resumes'][jobApplication.id].workExperience}','${divPage.dataMap['resumes'][jobApplication.id].hopeJob}','${divPage.dataMap['resumes'][jobApplication.id].hopeJob}','${divPage.dataMap['isBoughts'][jobApplication.id]}','${divPage.dataMap['users'][jobApplication.id].birth}')"
										...
										<td align="center" class="r8 stateTd" id="${jobApplication.id}">
											<c:choose>
												<c:when test="${empty jobApplication.applyStatus||jobApplication.applyStatus==0}">
													<c:choose>
												...
												<c:when test="${jobApplication.applyStatus==1}">
													<c:choose>
														<c:when test="${empty divPage.dataMap['limitPush'][jobApplication.id]||divPage.dataMap['limitPush'][jobApplication.id]==0}">
															<c:choose>
																<c:when test="${comType==0}">
												...
												<c:otherwise>
													<span class=" last_exe_btn_common last_exe_btn_invalid" title="已通知任职">已通知</span>
												</c:otherwise>
											</c:choose>  
										</td>
										<td align="center" class="r8 timeTd" id="${status.index}">
											<c:choose>
												<c:when
													test="${jobApplication.applyStatus>=1}">

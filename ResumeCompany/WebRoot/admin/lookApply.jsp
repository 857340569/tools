...
<script language="JavaScript">
	...
	function changeEmployState(status) {
		if($("input:checkbox[name=selectFlag]:checked").length<=0)
		{
			...
			alert($(".dataIds[mytag='testaaa'] ").length);
		}

	}
	var isShow = false;
	function tips(e, name, sex, phone, address, workExperience, hopework, selfAssess, state,birth) {
		...
		if (isShow) {
			...
			$("#selfAssess").text(checkData(selfAssess));
			isShow = true;
		}
	}
	...
	//弹出
	function treatments(uName,comName,sex,jobname,state) {
	...
	function sureTipButton(userName,state) {
		...
		//notifyType  面试/任职
		msgContent="&&您非常符合我们招聘"+$("#jobName").val()+"的要求，请您于"+$("#informtimes").val()+"到"+$("#tipAddress").val()+notifyType;
		return true;
	}
	//关闭弹出框
	...
	// 判断网页是否加载完成  
      ...
      //userName推送用户名
	 //uName，姓名
      function notifyUser(applyId,comId,comName,jobId,uName,userName,deviceType,state,jobname,sex)
      {
      	if("${comType}"=="0")
		{
			comId="${id}";
		...
		if(jobname.indexOf("-") != -1)
		{
      	...
      	treatments(uName,comName,sex,jobname,state);
      	...
      	$("#sendToUserBtn").click(function(){
      		...
      		if(checkInput)
      		{
      			var showConfirms="";
				if (state ==1){
					showConfirms="发送通知将会扣除6米粒服务费且通知后将不能取消,确定要发送通知吗?";
				}
				if (state == 2) {
							...
							<td height="215" valign="top">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<c:if test="${empty divPage.dataMap['jobApplications']}">
										<tr align="center">
											<td height="35" class="rd8" colspan="8"><font
									...
									<c:forEach items="${divPage.dataMap['jobApplications']}" var="jobApplication"
										...
										<tr align="center" id="${status.count}" height="29"
											...
											<td align="center" class="r8""><a
												href="resume!queryResumeByUserId?user.id=${jobApplication.userId}"> 
												...
												'${divPage.dataMap['resumes'][jobApplication.id].locationPlace}',
													'${divPage.dataMap['resumes'][jobApplication.id].workExperience}','${divPage.dataMap['resumes'][jobApplication.id].hopeJob}','${divPage.dataMap['resumes'][jobApplication.id].hopeJob}','${jobApplication.applyStatus}','${divPage.dataMap['users'][jobApplication.id].birth}')"
										...
										<td align="center" class="r8">
											  <c:choose>
												<c:when test="${empty divPage.dataMap['jobs'][jobApplication.id].name}">
														<c:choose>
												...
												<c:otherwise>
													${divPage.dataMap['jobs'][jobApplication.id].name}
												</c:otherwise>
											</c:choose>
										</td>
										<td align="center" class="r8">
										...
										<td align="center" class="r8 stateTd" id="${jobApplication.id}">
											<c:choose>
												<c:when
													test="${empty jobApplication.applyStatus||jobApplication.applyStatus==0}">
													<c:choose>
														<c:when test="${comType==0}">
															<a title="点击发送通知消息" href="javascript:notifyUser(${jobApplication.id},-1,'${name}','${divPage.dataMap['jobs'][jobApplication.id].id}','${divPage.dataMap['users'][jobApplication.id].name}','${divPage.dataMap['users'][jobApplication.id].userName}','${divPage.dataMap['users'][jobApplication.id].deviceType}',1,'${divPage.dataMap['jobs'][jobApplication.id].category}','${divPage.dataMap['users'][jobApplication.id].sex}');">未通知</a>
														</c:when>
														<c:otherwise>
															<a title="点击发送通知消息" href="javascript:notifyUser(${jobApplication.id},'${divPage.dataMap['companies'][jobApplication.id].id}','${divPage.dataMap['companies'][jobApplication.id].name}','${divPage.dataMap['jobs'][jobApplication.id].id}','${divPage.dataMap['users'][jobApplication.id].name}','${divPage.dataMap['users'][jobApplication.id].userName}','${divPage.dataMap['users'][jobApplication.id].deviceType}',1,'${divPage.dataMap['jobs'][jobApplication.id].category}','${divPage.dataMap['users'][jobApplication.id].sex}');">未通知</a>
														</c:otherwise>
													</c:choose>
												</c:when>
												<c:when test="${jobApplication.applyStatus==1}">
													<c:choose>
														<c:when test="${comType==0}">
															<a title="通知消息已发送，点击录用" href="javascript:notifyUser(${jobApplication.id},-1,'${name}','${divPage.dataMap['jobs'][jobApplication.id].id}','${divPage.dataMap['users'][jobApplication.id].name}','${divPage.dataMap['users'][jobApplication.id].userName}','${divPage.dataMap['users'][jobApplication.id].deviceType}',2,'${divPage.dataMap['jobs'][jobApplication.id].category}','${divPage.dataMap['users'][jobApplication.id].sex}');">已通知</a>
														</c:when>
														<c:otherwise>
															<a  title="通知消息已发送，点击录用" href="javascript:notifyUser(${jobApplication.id},'${divPage.dataMap['companies'][jobApplication.id].id}','${divPage.dataMap['companies'][jobApplication.id].name}','${divPage.dataMap['jobs'][jobApplication.id].id}','${divPage.dataMap['users'][jobApplication.id].name}','${divPage.dataMap['users'][jobApplication.id].userName}','${divPage.dataMap['users'][jobApplication.id].deviceType}',2,'${divPage.dataMap['jobs'][jobApplication.id].category}','${divPage.dataMap['users'][jobApplication.id].sex}');">已通知</a>
														</c:otherwise>
													</c:choose>
												</c:when>
												<c:otherwise>
													<a title="录用消息已发" style="text-decoration:none;"
														class="empState">已录用</a>
												</c:otherwise>
											</c:choose> 
										</td>
										<td align="center" class="r8 timeTd" id="${status.index}">
											<c:choose>
												<c:when
													test="${jobApplication.applyStatus==2}">

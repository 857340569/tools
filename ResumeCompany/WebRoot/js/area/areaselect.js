function locationCard(settings) {
    ...
    var cur_p = pInput.text() ? pInput.text() : null,
        cur_c = cInput.text() ? cInput.text() : null,
        cur_x = countyInputId ? (xInput.text() ? xInput.text() : null) : null;
    ...
    function showCityList(province) {
        ...
        hideProvinceList();
        hideCountyList();
        cur_p = pInput.text() ? pInput.text() : null;
        if (!cur_p || cur_p.length == 0) { 
        	return;
    ...
    function showCountyList() {
        ...
        cur_p = pInput.text() ? pInput.text() : null;
        cur_c = cInput.text() ? cInput.text() : null;
        if (!cur_p || cur_p.length == 0 ||cur_p=='-请选择-' || !cur_c || cur_c.length == 0 ||cur_c=='-请选择-') {
        	return; 
        }
